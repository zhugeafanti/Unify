...
import com.example.unicallback_demo.UFUniCallbackDispatcher;
...
public class UniCallbackTestServiceRegister {
    ...
    public static void setup(BinaryMessenger binaryMessenger, UniCallbackTestService impl) {
        ...
        {
            BasicMessageChannel<Object> channel =
                    new BasicMessageChannel<>(binaryMessenger, "com.didi.flutter.uni_api.UniCallbackTestService.doCallbackAction2", new StandardMessageCodec());
            if (impl != null) {
                UFUniAPI.registerModule(impl);
                channel.setMessageHandler((message, reply) -> {
                    Map<String, Object> wrapped = new HashMap<>();
                    try {
                        Map<String, Object> params = (Map<String, Object>) message;
                        String callbackName = (String) params.get("callback");
                        UniCallbackTestService.OnDoCallbackAction2Callback callback = new UniCallbackTestService.OnDoCallbackAction2Callback(binaryMessenger, callbackName, impl);
                        UFUniCallbackDispatcher.registerCallback(callbackName, callback);
                        impl.doCallbackAction2(callback);
                        wrapped.put("result", null);
                    } catch (Error | RuntimeException exception) {
                        wrapped.put("error", wrapError(exception));
                        Log.d("flutter", "error: " + exception);
