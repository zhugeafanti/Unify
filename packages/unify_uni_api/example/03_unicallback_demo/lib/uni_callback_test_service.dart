...
class UniCallbackTestService {
    ...
    static Future<void> doCallbackAction2(UniCallback<bool> callback) async  {
        callback.callbackName = 'UniCallbackTestService_doCallbackAction2_callback_${callback.hashCode}';
        uniCallbackCache['UniCallbackTestService_doCallbackAction2_callback_${callback.hashCode}'] = callback;
        final Map<String, dynamic> encoded = {};
        encoded["callback"] = 'UniCallbackTestService_doCallbackAction2_callback_${callback.hashCode}';
        const BasicMessageChannel<Object?> channel = BasicMessageChannel<Object?> (
            'com.didi.flutter.uni_api.UniCallbackTestService.doCallbackAction2', StandardMessageCodec());
        UniCallbackManager.getInstance();
        final Map<Object?, Object?>? replyMap =
            await channel.send(encoded) as Map<Object?, Object?>?;
        if (replyMap == null) {
            UniApi.trackError('UniCallbackTestService', 'com.didi.flutter.uni_api.UniCallbackTestService.doCallbackAction2', 'Unable to establish connection on channel : "com.didi.flutter.uni_api.UniCallbackTestService.doCallbackAction2" .');
            if (!kReleaseMode) {
                throw PlatformException(
                    code: 'channel-error',
                    message: 'Unable to establish connection on channel : "com.didi.flutter.uni_api.UniCallbackTestService.doCallbackAction2" .',
                    details: null,
                );
            }
        } else if (replyMap['error'] != null) {
            ...
            if (error.containsKey('message')) errorMsg += '[ ${error['message']?.toString() ?? ''} ]';
            if (error.containsKey('details')) errorMsg += '[ ${error['details']?.toString() ?? ''} ]';
            UniApi.trackError('UniCallbackTestService', 'com.didi.flutter.uni_api.UniCallbackTestService.doCallbackAction2', 'doCallbackAction2: $errorMsg);');
            if (!kReleaseMode) {
                throw PlatformException(
                    code: error['code'] as String,
                    message: error['message'] as String,
